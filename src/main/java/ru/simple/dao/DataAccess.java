...
public class DataAccess {
    ...
    public List<Cars> getAllCars() {
        ...
        switch (sort) {
            case "by_year_asc":
                result = session.createCriteria(Cars.class).addOrder(Order.asc("year")).list();
                break;
            case "by_year_desc":
                result = session.createCriteria(Cars.class).addOrder(Order.desc("year")).list();
                break;
            case "by_price_asc":
                result = session.createCriteria(Cars.class).addOrder(Order.asc("price")).list();
                break;
            case "by_price_desc":
                result = session.createCriteria(Cars.class).addOrder(Order.desc("price")).list();
                break;
            case "by_id_asc":
                result = session.createCriteria(Cars.class).addOrder(Order.asc("id")).list();
                break;
            case "by_id_desc":
                result = session.createCriteria(Cars.class).addOrder(Order.desc("id")).list();
                break;
            default:
                result = session.createCriteria(Cars.class).addOrder(Order.asc("id")).list();
                break;
